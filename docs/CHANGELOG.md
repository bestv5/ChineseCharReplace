...
- 自适应语境检测明确空白语义：空格/制表符/换行/控制字符不作为语境证据，也不使检测有判据——纯空白/换行窗口按键入字符补判，含有效内容的窗口忽略空白后判定（全角空格 U+3000 属中文标点区段，仍按中文语境证据处理）
- 替换改为两阶段执行：候选字符按键时暂存、下一拍（EDT 稍后）以独立撤销命令替换，视觉上替换延迟一拍生效；撤销序列为 [键入组][替换][键入组]，Ctrl+Z 单步撤销替换本身
...
- 自适应区域键入中文标点不再被自身污染为中文语境：语境窗口改为不含刚键入字符，按键入前的中英文语境决策（修复注释/字符串/纯文本区标点静默不替换）；窗口无判据时按键入字符补判，保持 ASCII 自定义键行首键入可替换
- 替换撤销粒度修复：替换推迟至键入命令结束后以独立撤销命令执行，Ctrl+Z 现在只撤销替换本身，不再连带撤销整段键入
